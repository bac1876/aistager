...
<body>
                ...
                <div>
                    <div class="floating-card p-6 sm:p-8">
                        ...
                        <div class="mb-6">
                            ...
                            <!-- URL Input Option -->
                            <div class="mt-4">
                                <p class="text-xs text-gray-500 mb-2">Or paste an image URL:</p>
                                <input type="url" id="imageUrlInput" placeholder="https://example.com/room.jpg" class="w-full p-3 rounded-lg text-sm">
                                <p class="text-xs text-blue-400 mt-1 mobile-tip hidden">💡 Tip: Using URLs is much faster on mobile networks!</p>
                            </div>
                        </div>
    ...
    <script>
        ...
        async function stageCurrentRoom() {
            ...
            try {
                // Check for pending task from previous timeout
                ...
                if (pendingTask) {
                    ...
                    // If task is less than 10 minutes old, try to resume it
                    if (age < 600000) {
                        ...
                        if (resumeData.success && resumeData.status === 'completed' && resumeData.images && resumeData.images.length > 0) {
                            // Previous task completed! Use its result
                            data = resumeData;
                            progressFill.style.width = '40%';
                            progressText.textContent = 'Found completed result from previous request!';
                            localStorage.removeItem('pendingKieTask');
                        } else if (resumeData.status === 'processing') {
                            // Task still processing, continue polling it
                            data = { success: true, status: 'processing', taskId: taskId };
                            progressFill.style.width = '40%';
                            progressText.textContent = 'Resuming previous task...';
                        } else {
                            // Task failed or not found, clear and create new one
                            localStorage.removeItem('pendingKieTask');
                        }
                    } else {
