...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Check both HTTP status AND response body code (Kie.ai returns 200 OK but with error codes in body)
    if (!createTaskResponse.ok || (taskData.code && taskData.code !== 200 && taskData.code !== 0)) {
      console.error('Kie.ai createTask error:', taskData);
      console.error('Response status:', createTaskResponse.status);
      console.error('Response body code:', taskData.code);
      console.error('Full response:', JSON.stringify(taskData, null, 2));

      // Handle specific error codes from response body
      if (taskData.code === 402) {
        throw new Error('402: Insufficient credits in Kie.ai account. Please top up at kie.ai');
      }
      if (taskData.code === 401) {
        throw new Error('401: Unauthorized - Invalid Kie.ai API key');
      }
      if (taskData.code === 429) {
        throw new Error('429: Rate limit exceeded. Please wait before trying again');
      }

      throw new Error(taskData.msg || taskData.message || taskData.error || `Failed to create task at Kie.ai (status ${createTaskResponse.status}, code ${taskData.code})`);
    }
