...
module.exports = async function handler(req, res) {
  ...
  // CRITICAL: Prevent caching - status changes frequently during polling
  ...
  try {
    ...
    console.log('Kie.ai status check FULL:', JSON.stringify(statusData, null, 2));
    console.log('Kie.ai status check summary:', {
      taskId,
      state: statusData.data?.state,
      code: statusData.code,
      hasResultJson: !!statusData.data?.resultJson,
      resultJsonLength: statusData.data?.resultJson?.length || 0
    });
    ...
    if (status === 'successful' || status === 'completed') {
      ...
      console.log('Extracted images:', resultImages);

      // If completed but no images found, return error instead of empty success
      if (resultImages.length === 0) {
        console.error('Task completed but no images found in response');
        return res.status(200).json({
          success: false,
          status: 'failed',
          error: 'Task completed but no images were returned. Please try again.'
        });
      }
